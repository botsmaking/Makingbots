...
def _resolve_host_url():
    for key in ("SHOP_URL", "WEBAPP_URL", "RENDER_EXTERNAL_URL"):
        u = (os.getenv(key) or "").strip().rstrip("/")
        if u:
            return u
...
# Shared login template + per-user registry.  IMPORTANT: request state must not
# live in module globals because FastAPI serves concurrent requests.  A global
# `db = ...` swap lets one user's request overwrite another user's active account
# mid-request, which is a common cause of random re-login/cart/payment failures.
_BASE_DB = db
_DB_CTX = ContextVar("sj_db", default=_BASE_DB)
_USER_CTX = ContextVar("sj_user", default=None)
_NS_CTX = ContextVar("sj_ns", default="default")
_PERSIST_LOCK = threading.Lock()

class _DBProxy:
    def __init__(self, ctx): self._ctx = ctx
    def _d(self): return self._ctx.get()
    def __getitem__(self, k): return self._d()[k]
    def __setitem__(self, k, v): self._d()[k] = v
    def __delitem__(self, k): del self._d()[k]
    def __iter__(self): return iter(self._d())
    def __len__(self): return len(self._d())
    def get(self, *a, **kw): return self._d().get(*a, **kw)
    def setdefault(self, *a, **kw): return self._d().setdefault(*a, **kw)
    def update(self, *a, **kw): return self._d().update(*a, **kw)
    def pop(self, *a, **kw): return self._d().pop(*a, **kw)
    def items(self): return self._d().items()
    def keys(self): return self._d().keys()
    def values(self): return self._d().values()

SEED = _BASE_DB
DEVICES = _BASE_DB["devices"]
db = _DBProxy(_DB_CTX)
...
def _start_telegram_bot_background():
    """Run bot.py long-polling in a daemon thread (same Render web service)."""
    global _bot_thread
    token = (os.getenv("BOT_TOKEN") or "").strip()
    if not token:
        # also allow bot_config.json
        try:
            cfg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bot_config.json")
            if os.path.isfile(cfg_path):
                with open(cfg_path, "r", encoding="utf-8") as f:
                    token = str((json.load(f) or {}).get("token") or "").strip()
        except Exception:
            token = ""
    if not token or token in ("PUT_TELEGRAM_BOT_TOKEN_HERE", "PASTE_YOUR_BOT_TOKEN_HERE"):
        print("[bot] skipped — set BOT_TOKEN env (or bot_config.json token)", flush=True)
        return
    # Ensure shop URL is visible to bot module
    if not (os.getenv("SHOP_URL") or os.getenv("WEBAPP_URL")):
        if HOST_URL and HOST_URL.startswith("http"):
            os.environ["SHOP_URL"] = HOST_URL
    try:
        import bot as _bot_mod
        if _bot_thread and _bot_thread.is_alive():
            return
        _bot_thread = threading.Thread(target=_bot_mod.main, name="tg-bot", daemon=True)
        _bot_thread.start()
        print("[bot] background polling started", flush=True)
    except Exception as e:
        print("[bot] failed to start:", e, flush=True)

@asynccontextmanager
async def _lifespan(app):
    _start_telegram_bot_background()
    yield

app = FastAPI(title="SJ Shop — Meesho Mini App", lifespan=_lifespan)
...
CURRENT_USER = None  # legacy compatibility; request code uses ContextVar below
CURRENT_NS = "default"  # legacy compatibility; request code uses ContextVar below
...
async def device_isolation_middleware(request: Request, call_next):
    ...
    if admin:
        if not (user and user.get("role") == "admin"):
            ...
            _NS_CTX.set(ns)
            _DB_CTX.set(DEVICES.setdefault(ns, _default_state()))
    ...
    if user:
        ...
        if user.get("role") == "admin":
            try:
                ...
                pass
        # Telegram Mini Apps can hand out a different browser/device storage
        # identifier after a refresh/update.  For Telegram users, key the shop
        # state by Telegram account, not by the volatile WebView device id, so
        # the same saved Meesho account is reused instead of asking for OTP again.
        if user.get("tg_id") is not None:
            ns = "u%s:tg" % int(user.get("id") or 0)
            if ns not in DEVICES:
                # Migrate the newest legacy namespace for this user, if present.
                legacy = next((k for k in reversed(list(DEVICES.keys()))
                               if k.startswith("u%s:" % int(user.get("id") or 0))
                               and (DEVICES[k].get("accounts") or [])), None)
                DEVICES[ns] = copy.deepcopy(DEVICES[legacy]) if legacy else _default_state()
        else:
            ns = "u%s:%s" % (int(user.get("id") or 0), did)
    else:
        ns = "anon:" + did
...
def _device_id():
    ns = _NS_CTX.get() or "default"
    return (ns.rsplit(":", 1)[-1] or "default")

# Flat deploy: index.html beside app.py (or public/index.html)
_BASE_DIR = os.path.dirname(os.path.abspath(__file__))
_INDEX_CANDIDATES = [
    os.path.join(_BASE_DIR, "index.html"),
    os.path.join(_BASE_DIR, "public", "index.html"),
]
try:
    os.makedirs(os.path.join(_BASE_DIR, "state"), exist_ok=True)
except Exception:
    pass
...
async def serve_index():
    path = next((p for p in _INDEX_CANDIDATES if os.path.isfile(p)), None)
    if not path:
        return JSONResponse(
            {"error": "index_missing", "message": "Upload index.html next to app.py"},
            status_code=500,
        )
...
async def api_index():
    ...
    return {
        ...
        "endpoints": routes,
        "usage": "Telegram WebApp at GET /. Set BOT_TOKEN (+ optional SHOP_URL); bot auto-starts with the web process on Render.",
    }
...
async def api_auth_otp_verify(data: dict = None, request: Request = None):
    ...
    # Switch only this request's ContextVar; never mutate a process-global db.
    _NS_CTX.set(ns)
    _DB_CTX.set(ust)
    ...
    if acc:
        ...
        acc = new_acc
    # Never bind a made-up ₹180/₹150 offer. Meesho's live checkout is the
    # source of truth; use its returned first-order offer when available.
    if is_new:
        try:
            live_fod = await _account_real_fod(acc)
            if live_fod.get("ok") and live_fod.get("offer"):
                acc["bound_offer"] = dict(live_fod["offer"])
                ust["picked_offer"] = dict(live_fod["offer"])
            else:
                acc.pop("bound_offer", None)
                ust["picked_offer"] = None
        except Exception:
            acc.pop("bound_offer", None)
            ust["picked_offer"] = None
    _persist()
...
async def api_accounts_login_verify(data: dict):
    ...
    if res["ok"]:
        ...
        if is_new:
            # Do not fabricate a first-order discount. The live Meesho checkout
            # is the source of truth for the amount/offer actually payable.
            new_acc["bound_offer"] = None
            db["picked_offer"] = None
...
async def _send_preorders(body, headers, uid):
    """Create a real Meesho preorder and recover from a rotating checkout
    session/price. UPI retries use a fresh payment-summary session so the
    amount sent to Meesho is the current prepaid amount, not the stale COD total."""
    def _send(cs, amt):
        b = dict(body)
        b["cart_session"] = cs
    ...
    is_upi = str(body.get("payment_method") or body.get("payment_method_type") or "").upper() == "UPI"
    d = {}
    final_cs = body.get("cart_session")
    final_amt = body.get("customer_amount")

    for attempt in (1, 2, 3):
        cs = body.get("cart_session") or ""
        amt = body.get("customer_amount")

        if attempt > 1:
            # Re-sync the current local bag before rebuilding checkout. This
            # prevents CART_UPDATED when the server rotated the buy-now session
            # between cart/add, location binding and preorders.
            try:
                local_items = [x for x in (db.get("cart", {}).get("items") or [])
                               if x.get("product_id") and x.get("supplier_id") and x.get("variation_id")]
                if local_items:
                    add = await _real_cart_add_many(local_items, "")
                    if add and add.get("success"):
                        cs = add.get("cart_session") or cs
            except Exception as exc:
                _dump_preorders("preorders_retry_resync_error", error=str(exc))

            fresh = await _fresh_checkout_state(need_paymentinfo=is_upi)
            if not fresh:
                _dump_preorders("preorders_retry_no_fresh", attempt=attempt)
                return False, d, None, None, None
            cs = fresh["cs"]
            # UPI must use Meesho's live prepaid amount. COD uses the checkout total.
            amt = fresh.get("upi_amount") if is_upi else fresh.get("amt")
            if amt is None:
                amt = fresh.get("amt")
        ...
        if not resp:
            _dump_preorders("preorders_response", status=None, attempt=attempt)
            return False, d, final_cs, None, final_amt
        _dump_preorders("preorders_response", status=resp.status_code, attempt=attempt)
        try:
            d = resp.json()
        except Exception:
            d = {}
        ...
        if code.upper() in _CART_CHANGED_CODES and attempt < 3:
...
async def api_place_cod(data: dict = None):
    ...
    # Creating a UPI preorder is not the same as completing payment.  Keep the
    # local first-order flag intact until Meesho confirms SUCCESS, otherwise a
    # failed/abandoned payment incorrectly burns the first-order state.
    it = (st["items"] or [{}])[0]
    order = {
        "order_num": order_num,
...
def _extract_official_upi_payload(d: dict, amount) -> dict:
    """Extract only payment instructions actually returned by Meesho/Juspay.
    Never invent a merchant VPA, QR, or transaction id: a generated fallback QR
    can send money to the wrong payee and cannot be reconciled reliably."""
    out = {"upi_uri": "", "qr_base64": "", "qr_url": "", "package_name": "com.google.android.apps.nbu.paisa.user",
           "juspay_order_id": "", "txn": {}}
    seen=set()
    def walk(x):
        ...
        if isinstance(x, dict):
            for k,v in x.items():
                kl=str(k).lower()
                if isinstance(v, str):
                    if kl in ("qr_base64_string","qr_base64","qr_image","qr_code") and len(v)>40 and not out["qr_base64"]:
                        if v.startswith(("data:image/", "iVBOR", "/9j/")):
                            out["qr_base64"] = v
                    if kl in ("qr_url","qr_image_url","qr_code_url") and v.startswith(("https://", "http://")) and not out["qr_url"]:
                        out["qr_url"] = v
                    if kl in ("upi_intent_url","intent_url","upi_uri","upi_link","deep_link") and v.startswith(("upi://","upic:","https://")) and not out["upi_uri"]:
                        out["upi_uri"] = v
                    if kl in ("order_id","juspay_order_id","payment_order_id") and v and not out["juspay_order_id"]:
                        out["juspay_order_id"] = v
                    if kl == "package_name" and v.startswith("com."):
                        out["package_name"] = v
            ...
            for v in x: walk(v)
    walk(d or {})
    # Preserve the transaction object when the API supplies one.
    tp = d.get("qr_transaction_params") or d.get("juspay_transaction_params") or d.get("transaction_params") if isinstance(d,dict) else None
    ...
    if isinstance(tp, dict):
        ...
        if isinstance(payload, dict):
            out["txn"]={k:payload.get(k) for k in ("order_id","transaction_id","client_transaction_id","client_auth_token","action","amount","currency") if payload.get(k) is not None}
    if not out["juspay_order_id"]:
        out["juspay_order_id"] = str((out["txn"] or {}).get("order_id") or "")
    return out
...
async def api_order_pay_online(data: dict = None):
    ...
    # 1) REAL cart session + address + live UPI amount. The online price can be
    # different from COD, so use paymentinfo for UPI and pass the prepaid amount
    # returned by Meesho to preorders. If the checkout session rotates, the
    # preorders helper rebuilds it automatically.
    ...
    # The amount charged by the UPI preorder must match the current prepaid
    # amount, not the COD total shown in the review cart.
    preorder_amount = upi_amount if upi_amount is not None else amt
    ...
    body = {
        ...
        "is_selling_to_customer": False, "cart_session": cs, "vpa": None,
        "address_id": int(addr["id"]), "direct_wallet_token": None,
        "customer_amount": preorder_amount, "upi_package_name": "com.google.android.apps.nbu.paisa.user",
    ...
    order_num = str(d["order_num"])
    extracted = _extract_official_upi_payload(d, amt)
    upi_uri = extracted["upi_uri"]
    package_name = extracted["package_name"]
    txn = extracted["txn"]
    jtid = extracted["juspay_order_id"]
    qr_base64 = extracted["qr_base64"]
    qr_url = extracted.get("qr_url") or ""

    # Meesho must provide the actual payment instruction.  Do not construct a
    # fake MEESHO VPA/QR when the preorder response omitted it.
    if not upi_uri and not qr_base64:
        _dump_preorders("missing_official_payment_payload", order_num=order_num, response=d)
        return {"ok": False, "live": True, "error": "payment_payload_missing",
                "message": "Meesho created the payment order but did not return its official UPI QR/intent. Please retry once.",
                "order_num": order_num}
    ...
    if upi_uri and "pa=" in upi_uri:
        try:
            ...
            merchant_vpa = parse_qs(urlparse(upi_uri.replace("upic:", "")).query).get("pa", [""])[0] or ""
        except Exception:
            merchant_vpa = ""
    ...
    order = {
        ...
        "device_id": _device_id(),
        "upi_uri": upi_uri,
        "qr_image": (("data:image/png;base64," + qr_base64) if (qr_base64 and not str(qr_base64).startswith("data:")) else (qr_base64 or qr_url)),
    ...
    resp_out = {
        ...
        "redirect_url": (upi_uri if (upi_uri and str(upi_uri).startswith(("upi://", "upic:"))) else ""),
...
async def _finalize_paid_online_order(order_num, cart_session=None):
    """Finalize a UPI order exactly once after Meesho reports payment SUCCESS.
    This keeps the account's first-order flag, usage counter, wallet fee and
    local cart in sync even when payment completes after the initial QR screen."""
    o = next((x for x in db["orders"] if str(x.get("order_num")) == str(order_num)), None)
    if not o:
        return False
    already = str(o.get("status_id") or "").upper() in {"STATUS_ID_ORDERED", "ORDERED"} or bool(o.get("payment_finalized"))
    o["status_id"] = "STATUS_ID_ORDERED"
    o["status_text"] = "Order Placed"
    o["status_color"] = "#038D63"
    o["payment_finalized"] = True
    if not already:
        acc = _active_account()
        if acc and o.get("is_first_order"):
            acc["is_first_order"] = False
            acc["order_placed"] = True
        _persist()
        await _clear_cart_after_order(cart_session or o.get("cart_session") or db["cart"].get("cart_session") or "", [])
        usr = _current_user()
        fee = float(_load_settings().get("per_order_price") or 0)
        if usr:
            _debit_wallet(usr, fee)
            _charge_order(usr)
    else:
        _persist()
    return True
...
async def api_order_payment_status(data: dict = None):
    ...
    if state and state.get("live") and state.get("state") != "unknown":
        ...
        if state["state"] == "confirmed":
            await _finalize_paid_online_order(onum, cs)
