...
def _client_kwargs(timeout=20.0):
    ...
    proxy = _proxy_url()
    if proxy:
        # httpx 0.28 uses `proxy` (singular); `proxies` raises TypeError and
        # can make otherwise healthy Meesho requests look like login/payment failures.
        kw["proxy"] = proxy
    return kw
...
async def fetch_fod(device: dict = None) -> dict:
    ...
    try:
        ...
        pass
    return {"ok": False, "message": "Meesho did not return a live first-order offer right now."}
